...
namespace Day5
{
        ...
        private int RunNextSection(int startIndexPosition, int InputParameter, out int OutputParameter)
        {
            ...
            switch (opCodeParser.FunctionMode)
            {
                case OpCodeMode.Addision:
                    ...
                    IntCodeComputer.Addision(this._IntcodeList, opCodeParser, startIndexPosition);
                    NextIndexStartPosition = startIndexPosition + 4;
                    break;

                case OpCodeMode.Multiplication:
                    //NewValueToCompute = this._IntcodeList[this._IntcodeList[startIndexPosition + 1]] * this._IntcodeList[this._IntcodeList[startIndexPosition + 2]];
                    IntCodeComputer.Multiplication(this._IntcodeList, opCodeParser, startIndexPosition);
                    NextIndexStartPosition = startIndexPosition + 4;
                    break;

                case OpCodeMode.Input:
                    IntCodeComputer.Input(this._IntcodeList, startIndexPosition, InputParameter);
                    NextIndexStartPosition = startIndexPosition + 2;
                    break;

                case OpCodeMode.Output:
                    OutputParameter = IntCodeComputer.Output(this._IntcodeList, opCodeParser, startIndexPosition);
                    NextIndexStartPosition = startIndexPosition + 2;
    ...
    class IntCodeComputer
    {
        ...
        public static void Addision(List<int> IntcodeList, OpCodeParser opCodeData, int startIndexPosition)
        {
            int paramOneValue = 0;
            int paramTwoValue = 0;
        ...
        /// <param name="opCodeData">information about how each parameter should operate (Position mode or Immediate mode)</param>
        /// <param name="startIndexPosition">the index position in IntcodeList the OpCode starts at that we are currently looking at</param>
        public static void Multiplication(List<int> IntcodeList, OpCodeParser opCodeData, int startIndexPosition)
        {
            int paramOneValue = 0;
            int paramTwoValue = 0;
        ...
        public static void Input(List<int> IntcodeList, int startIndexPosition, int InputValue)
        {
            IntcodeList[IntcodeList[startIndexPosition + 1]] = InputValue;
        }

        public static int Output(List<int> IntcodeList, OpCodeParser opCodeData, int startIndexPosition)
        {
            if(opCodeData.ParameterModeOne == OpCodeParser.ParameterPositionMode)
    ...
    enum OpCodeMode { Addision = 1, Multiplication = 2, Input = 3, Output, Halt = 99}
}
